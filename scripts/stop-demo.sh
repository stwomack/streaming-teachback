#!/usr/bin/env bash
# Tear everything down: bridge, worker, and the Temporal dev server (only if we
# started it — a pre-existing server on 7233 is left alone).
...
sleep 1
for f in bridge worker temporal; do
